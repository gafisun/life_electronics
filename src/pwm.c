...
void set_all_freq() {
    int note_num = 0;
    for (int i = 0; i < 32; i++) {
        if(matrix[i][col_idx] == 1) note_num++;
    ...
    col_idx = col_idx == 31 ? 0 : col_idx+1;
}
...
void pwm_audio_handler() {
    ...
    int samp1 = (wavetable[offset0 >> 16] + wavetable[offset1 >> 16] + wavetable[offset2 >> 16] + wavetable[offset3 >> 16]) * envelope[envelope_index];
